...
public class ItemSet {
	...
	public ItemSet(ArrayList<String> items2) {
		items = items2;
	}
	...
	public boolean sharesPrefixExceptLastOne(ItemSet fk_j) {
		for (int i = 0; i < this.size() - 1; ++i) {
			if (this.get(i).compareTo(fk_j.get(i)) != 0) {
				return false;
			}
		}
		return true;
	...
	public ItemSet getPrefixMergedItemSet(ItemSet fk_j) {
		...
		if (this.get(len - 1).compareTo(fk_j.get(len - 1)) < 0) {
	...
	public int compareToExcludingIndex(ItemSet itemSet, int ignorePosition) {
		for (int i = 0; i < this.size(); ++i) {
			...
			int compareValue = this.get(i).compareTo(itemSet.get(i));
			if (compareValue != 0) {
				return compareValue;
			}
		}
